...
namespace DoAn2.Areas.Admin.Controllers
{
    ...
    public class HomeAdminController : Controller
    {
        ...
        {
            var lstSanPham=db.SANPHAM.ToList();
        ...
        public ActionResult ThemSanPhamMoi(SANPHAM sanPham)
        {
            if (ModelState.IsValid)
            {
                ...
                var existingProduct = db.SANPHAM.FirstOrDefault(sp => sp.MA_SP == sanPham.MA_SP);
        ...
        public ActionResult XoaSanPham(string maSanPham)
        {
            ...
            var sanPham = db.SANPHAM.Find(maSanPhamInt);
            if (sanPham == null)
            {
                return HttpNotFound();
            }
            ...
            bool chiTietHoaDonTonTai = db.CTDH.Any(ct => ct.MA_SP == maSanPhamInt);
        ...
        public ActionResult XoaLoaiSanPham(int? maLoai)
        {
            ...
            var hasSanpham = db.SANPHAM.Any(sp => sp.MALOAI == maLoai);
            if (hasSanpham)
            {
        ...
        public ActionResult ChiTietDatHang(int? id)
        {
            ...
            var order = db.DON_DAT_HANG
                .Include("KHACH_HANG")
                .Include("CTDH.SANPHAM")  // Include chi tiết đơn hàng và sản phẩm
            ...
            decimal totalAmountWithoutShipping = order.CTDH.Sum(item => (item.SOLUONG ?? 0) * (item.SANPHAM.GIAMON ?? 0));
            // Giả định phí ship là 20,000 VND nếu tổng số lượng sản phẩm ít hơn 5, nếu không thì miễn phí ship
            decimal shippingFee = order.CTDH.Sum(item => item.SOLUONG ?? 0) < 5 ? 20000 : 0;
        ...
        .Where(ddh => ddh.NGAY_GIODAT.HasValue && ddh.TRANGTHAI_DH == "Hoàn thành")
        .AsEnumerable()
        .GroupBy(ddh => ddh.NGAY_GIODAT.Value.Date)
        .Select(g => new
        {
            Ngay = g.Key,
            TongDoanhThu = g.Sum(ddh => ddh.CTDH.Sum(ct => (ct.SOLUONG ?? 0) * ct.SANPHAM.GIAMON) + ddh.ShippingFee)
        })
        .ToList();
